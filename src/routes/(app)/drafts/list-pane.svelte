<script lang="ts">
	...
	const selectedOutgoingMessageId = $derived.by((): OutgoingMessageId | null => {
		const match = $page.url.pathname.match(/^\/drafts\/(o\d+)$/);
...
<ScrollArea class="flex-1 overflow-hidden border-t border-border">
	{#each draftMessages as msg (msg.id)}
		...
		>
			<div class="flex items-center gap-2">
				<span class="flex-1 truncate text-sm font-medium">{msg.subject ?? 'Draft message'}</span>
				{#if msg.createdAt}
	...
	{#if draftMessages.length === 0}
		<div class="flex items-center justify-center p-8 text-sm text-muted-foreground">No drafts</div>
	{/if}
</ScrollArea>
